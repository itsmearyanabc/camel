...
export default async function Home() {
  ...
  return <div className={styles.page}>
    <header className={styles.nav}>
      <Link href="/" className="brand-lockup"><img src="/logo.jpg" alt="Camel971BOYZ" className="brand-logo" style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', boxShadow: '0 2px 8px rgba(189,123,28,0.3)' }} /></Link>
    ...
    <main>
      <section className={styles.hero} id="discover">
        <div className={styles.copy}><span className={styles.eyebrow}>◆ Private marketplace</span><h1>The <span>Camel971BOYZ</span> experience, refined.</h1><p>Discover a clean, secure way to browse products, manage your wallet, and follow every order — built around effortless access.</p><div className={styles.heroActions}><Link href={dashboardHref} className="btn btn-primary">{session ? "Open dashboard" : "Create account"} <span>→</span></Link>{!session && <Link href="/auth/login" className="btn btn-secondary">Sign in</Link>}</div></div>
        <div className={styles.visual} aria-hidden="true"><img src="/logo.jpg" alt="Camel971Hero" className={styles.heroImg} style={{ borderRadius: '50%', objectFit: 'cover' }} /><div className={`${styles.floatCard} ${styles.topCard}`}><span>Secure wallet</span><strong>Private by design</strong></div><div className={`${styles.floatCard} ${styles.bottomCard}`}><span>Always available</span><strong>24/7 bot access</strong></div></div>
      </section>
      <section className={styles.botBanner} id="access"><div className={styles.botInfo}><span className={styles.botIcon}>◌</span><div><strong>Shop 24/7 with the Camel971Bot</strong><p>Instant, secure access to your account anytime, anywhere.</p></div></div><a className="btn btn-primary btn-sm" href="https://t.me/Camel971Boys_bot" target="_blank" rel="noreferrer">Talk to bot →</a></section>
